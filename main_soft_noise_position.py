...
def get_new_adj_soft_position(feed_dict, sess, model, noised_index, adj_new, k, num_node):
    x_tilde = model.x_tilde.eval(session=sess, feed_dict=feed_dict)
    new_adj = adj_new.copy()
    new_adj_flatten = np.squeeze(new_adj.toarray().flatten())
    noised_index = np.where(new_adj_flatten == 1)
    noised_index = np.array(noised_index)[0]
...
def test_one_graph(adj , adj_orig, features_csr, num_node, k_num ,model,placeholders, sess, feed_dict):
    ...
    if k_num !=0:
        ...
        new_adj = get_new_adj_soft_position(feed_dict, sess, model,noised_indexes, adj_new, k_num, num_node)
    else:
        new_adj = adj_clean
