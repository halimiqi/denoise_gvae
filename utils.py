...
def WL_no_label(clean_adj, modified_adj):
  ...
  #####              ############
    #WL_clean = weisfeilerlehmankernel([G_clean, G_modified], node_label = "node_label",
    #                                    base_kernel = "sp")
...
def WL(clean_adj, modified_adj, y_label):
  ...
  #####              ############
    ...
    #                                    base_kernel = "sp")
    WL_clean = compute_mle_wl_kernel([G_clean, G_modified], 20)
    #WL_clean = weisfeilerlehmankernel([G_clean, G_modified])
    print(WL_clean)
    return float(WL_clean[0,1]/(math.sqrt(WL_clean[0,0])* math.sqrt(WL_clean[1,1])))
if __name__ == "__main__":
    ...
    wl = weisfeilerlehmankernel([graph1,graph2],base_kernel = "edge" )
    print(wl)
