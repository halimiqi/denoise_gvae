...
from mask_gvae_oldold import mask_gvae
...
def train():
    ...
    placeholders = {
        ...
        'node_mask':tf.placeholder(tf.int32)
    }
    # build models
    model = None
...
def test_one_graph(adj , adj_orig, features_csr, num_node ,model,placeholders, sess, feed_dict):
    ...
    psnr = PSNR(adj_clean, new_adj_sparse)
    y_label = y_train + y_val + y_test
    wls = WL_no_label(adj_clean, new_adj_sparse)
    return psnr, wls
